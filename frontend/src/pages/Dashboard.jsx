...
const FinanceApp = ({ usuario, onLogout }) => {
  ...
  const carregarDados = async () => {
    ...
    try {
      // Carregar transações (código existente)
      const resTransacoes = await fetch(`${API_URL}/api/transacoes/${usuario.id}`, {
        headers: getAuthHeaders()
      });
      if (resTransacoes.ok) {
        const dados = await resTransacoes.json();
      ...
      try {
        const resCategorias = await fetch(`${API_URL}/api/categorias/${usuario.id}`, {
          headers: getAuthHeaders()
        });
        if (resCategorias.ok) {
      ...
      try {
        const resContas = await fetch(`${API_URL}/api/contas/${usuario.id}`, {
          headers: getAuthHeaders()
        });
        if (resContas.ok) {
      ...
      // Carregar caixinhas (código existente mantém)
      const resCaixinhas = await fetch(`${API_URL}/api/caixinhas/${usuario.id}`, {
        headers: getAuthHeaders()
      });
      if (resCaixinhas.ok) {
        const dadosCaixinhas = await resCaixinhas.json();
        setCaixinhas(dadosCaixinhas);
      }
    } catch (err) {
...
const renderConfiguracoes = () => (
  <div className="pb-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
    ...
    <div className="mb-6 pt-4">
      ...
      <p className="text-gray-400">Gerencie categorias, contas e preferências</p>
    </div>
        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-500 p-6">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="bg-white text-blue-600 hover:bg-blue-50 px-4 py-3 rounded-lg font-bold flex items-center gap-2 transition-all shadow-lg hover:shadow-xl active:scale-95 min-h-[44px]"
            >
              <PlusCircle size={20} />
              Nova Categoria
            </button>
        ...
        <div className="p-6">
          {loadingCategorias ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-xl font-bold mb-2">Nenhuma categoria customizada</h3>
              <p className="text-gray-400 mb-6">
                Crie sua primeira categoria personalizada para começar
              </p>
              <button
                onClick={() => {
                  setCategoriaEdit(null);
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-4 mb-6">
                <div className="bg-gray-700 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-red-400">
                    {categoriasCustom.filter(c => c.tipo === 'fixo').length}
                  </div>
                  <div className="text-sm text-gray-400 mt-1">Fixas</div>
                </div>
                <div className="bg-gray-700 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-yellow-400">
                    {categoriasCustom.filter(c => c.tipo === 'variavel').length}
                  </div>
                  <div className="text-sm text-gray-400 mt-1">Variáveis</div>
                </div>
                <div className="bg-gray-700 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-green-400">
                    {categoriasCustom.filter(c => c.tipo === 'receita').length}
                  </div>
              ...
              {['fixo', 'variavel', 'receita'].map(tipo => {
                const categsTipo = categoriasCustom.filter(c => c.tipo === tipo);
                if (categsTipo.length === 0) return null;

                return (
                  <div key={tipo} className="mb-6 last:mb-0">
                    <h3 className="text-sm font-bold text-gray-400 uppercase mb-3">
                      {tipo === 'fixo' && '📌 Gastos Fixos'}
                      {tipo === 'variavel' && '📊 Gastos Variáveis'}
                      {tipo === 'receita' && '💰 Receitas'}
                      <span className="ml-2 text-gray-500">({categsTipo.length})</span>
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                      {categsTipo.map(cat => (
                        <div
                          key={cat.id}
                          className="bg-gray-700 hover:bg-gray-650 rounded-lg p-4 flex items-center gap-3 transition-all group"
                        >
                          <div
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="font-bold truncate">{cat.nome}</div>
                            <div className="text-sm text-gray-400 capitalize">{cat.tipo}</div>
                          </div>
                          <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              ...
                              className="p-2 hover:bg-blue-600 rounded-lg transition-colors min-h-[40px] min-w-[40px]"
                              title="Editar"
                            >
                              <Edit2 size={16} />
                            </button>
                            <button
                              onClick={async () => {
                                if (!confirm(`Deletar categoria "${cat.nome}"?`)) return;
                                
                                try {
                                  const res = await fetch(`${API_URL}/api/categorias/${cat.id}`, {
                                    method: 'DELETE',
                                    headers: getAuthHeaders()
                                  });
                              ...
                              className="p-2 hover:bg-red-600 rounded-lg transition-colors min-h-[40px] min-w-[40px]"
                              title="Deletar"
                            >
                              <Trash2 size={16} />
                            </button>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-green-100 mt-1">
                Gerencie seus cartões, contas bancárias e controle de gastos
              </p>
            </div>
            <button
              onClick={() => {
              ...
              className="bg-white text-green-600 hover:bg-green-50 px-4 py-3 rounded-lg font-bold flex items-center gap-2 transition-all shadow-lg hover:shadow-xl active:scale-95 min-h-[44px]"
            >
              <PlusCircle size={20} />
              Nova Conta
            </button>
        ...
        <div className="p-6">
          {loadingContas ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 mb-6">
                Adicione seus cartões e contas para melhor controle financeiro
              </p>
              <button
                onClick={() => {
                  setContaEdit(null);
                ...
                className="bg-green-600 hover:bg-green-700 px-6 py-3 rounded-lg font-bold inline-flex items-center gap-2"
              >
                <PlusCircle size={20} />
            ...
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
                {[
                  { tipo: 'cartao_credito', label: 'Crédito', icon: '💳', color: 'blue' },
                  { tipo: 'cartao_debito', label: 'Débito', icon: '💳', color: 'purple' },
                  { tipo: 'conta_corrente', label: 'Corrente', icon: '🏦', color: 'yellow' },
                  { tipo: 'poupanca', label: 'Poupança', icon: '🏦', color: 'green' },
                  { tipo: 'dinheiro', label: 'Dinheiro', icon: '💵', color: 'orange' },
                ].map(({ tipo, label, icon, color }) => {
                  const count = contas.filter(c => c.tipo === tipo).length;
                  return (
                    <div key={tipo} className="bg-gray-700 rounded-lg p-3 text-center">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {contas.map(conta => (
                  <div
                    key={conta.id}
                    className="bg-gray-700 hover:bg-gray-650 rounded-xl p-5 transition-all group relative overflow-hidden"
                  >
                    {/* Background decorativo */}
                    <div 
                      className="absolute inset-0 opacity-5"
                      style={{ 
                        background: `linear-gradient(135deg, ${conta.cor} 0%, transparent 100%)`
                      }}
                    />
                    ...
                    <div className="relative">
                      <div className="flex items-start gap-3 mb-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="font-bold text-lg truncate">{conta.nome}</div>
                          <div className="text-sm text-gray-400">
                            {conta.tipo === 'cartao_credito' && '💳 Cartão de Crédito'}
                            {conta.tipo === 'cartao_debito' && '💳 Cartão de Débito'}
                            {conta.tipo === 'conta_corrente' && '🏦 Conta Corrente'}
                            {conta.tipo === 'poupanca' && '🏦 Poupança'}
                            {conta.tipo === 'dinheiro' && '💵 Dinheiro'}
                          </div>
                        </div>
                      </div>
                      ...
                      {conta.tipo !== 'cartao_credito' && conta.saldo_atual !== undefined && (
                      ...
                      <div className="flex gap-2 pt-3 border-t border-gray-600">
                        <button
                          ...
                          className="flex-1 bg-blue-600 hover:bg-blue-700 px-3 py-2 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition-all min-h-[40px]"
                        >
                          <Edit2 size={14} />
                          Editar
                        </button>
                        <button
                          onClick={async () => {
                            if (!confirm(`Deletar conta "${conta.nome}"?`)) return;
                            
                            try {
                              const res = await fetch(`${API_URL}/api/contas/${conta.id}`, {
                                method: 'DELETE',
                                headers: getAuthHeaders()
                              });
                          ...
                          className="px-3 py-2 bg-red-600 hover:bg-red-700 rounded-lg transition-all min-h-[40px] min-w-[40px]"
                          title="Deletar"
                        >
                          <Trash2 size={14} />
                        </button>
      ...
      <div className="bg-gray-800 rounded-xl p-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between p-4 bg-gray-700 rounded-lg">
            ...
            >
              <div
                className={`w-6 h-6 bg-white rounded-full transition-all shadow-lg ${
          ...
          <button
            onClick={onLogout}
            className="w-full bg-red-600 hover:bg-red-700 p-4 rounded-lg font-bold flex items-center justify-center gap-2 transition-all min-h-[52px]"
          >
            <LogOut size={20} />
            Sair da Conta
          </button>
  ...
  className={`px-4 py-2.5 rounded-lg text-sm whitespace-nowrap min-h-[44px] flex-shrink-0 transition-all ${tela === 'configuracoes' ? 'bg-blue-600 scale-105' : 'bg-gray-700'}`}
>
  ⚙️ Configurações
</button>
          </div>
...
{tela === 'configuracoes' && renderConfiguracoes()}
